...
export class UserState {
  ...
  getUnreadBooks() {
    return this.allBooks.filter((book) => !book.started_reading_on)
      .toSorted((a, z) => new Date(z.created_at).getTime() - new Date(z.created_at).getTime())
      .slice(0, 9);
  }
  ...
  getFavoriteGenre() {
    ...
    const mostCommonGenre = Object.keys(genreCounts).reduce((a, z) => genreCounts[a] > genreCounts[z] ? a : z);
